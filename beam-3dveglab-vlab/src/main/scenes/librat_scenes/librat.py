...
class dolibradtran:
  ...
  def main(self, args):
    ...
    if not VLAB.fileExists(plotfile):
      plotfilefp = VLAB.openFileIfNotExists(plotfile)
    else:
    ...
    for a, aa in enumerate(angt):
      ...
      for f in [f for f in VLAB.listdir(opdir) if f.startswith('op.')]:
        vzz = f.split('_')[-4]
        vaa = f.split('_')[-3]
        szz = f.split('_')[-2]
        saa = f.split('_')[-1]
